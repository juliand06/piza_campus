# Examen Práctico: Base de Datos - Pizza del Campus
**Presentado por:** Andrey Julián Ricaurte Durán
**Motor:** MySQL

## Descripción General del Proyecto
Este proyecto contiene el diseño y la implementación de una base de datos relacional para gestionar la pizzería "Pizza del Campus". El sistema resuelve los problemas de desorganización al centralizar el inventario de productos (elaborados y no elaborados), los ingredientes, la creación de combos, el registro de clientes y la facturación de pedidos con opciones de personalización (adiciones extras).

## Instrucciones de Ejecución
Para probar la base de datos en un entorno local, sigue estos pasos:

1. Abre MySQL Workbench (o cualquier cliente MySQL que prefieras).
2. Abre un nuevo script SQL.
3. Ejecuta primero el código de creación de tablas (DDL) para generar la estructura y las llaves foráneas.
4. Ejecuta los Triggers incluidos. Estos son vitales porque automatizan la suma del total de los pedidos cada vez que se inserta un producto o adición.
5. Ejecuta el script de inserción de datos (DML) para poblar la base de datos con clientes, productos y pedidos de prueba.
6. Finalmente, ejecuta las consultas que se documentan a continuación para verificar los reportes.

---

## Consultas SQL y Resultados

A continuación se listan las 20 consultas requeridas para el examen, explicando de forma breve la lógica usada para llegar al resultado y la salida esperada según los datos de prueba.

### 1. Productos más vendidos
**Lógica:** Se unifican (con UNION ALL) los productos vendidos por separado en la tabla `detalles_pedido` y los que se venden dentro de un combo a través de la tabla `combo_producto`, agrupando por producto para sumar el total real despachado.
```sql
